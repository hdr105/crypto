<?php
require_once("header.php");
?>

<!-- START Row -->
<div class="row pt30">
  <!-- START col 12 -->
  <div class="col-md-8">
    CMT Cryptocurrency Market Capitalizations

    <!-- END Col4 -->
  </div>
  <!-- END Col 12-->
  <div class="col-md-4 currency_select_box">
    <select id="MCCLMC-currencies-select-box"></select>
  </div>

</div>
<!-- END Row -->


<!-- START Magen plugin DOM -->
<!-- NOTE: This Important your must add it in your project -->
                    <!-- Your can change the id="magen-plugin" to other id="<your-new-id-name>" but your must add the same name in your main javascript file
                      $('#<your-new-id-name>').magenCryptoLiveMarketCap({...})-->
                        <div id="magen-plugin" class="pt30 pb30"></div> 
                        <!-- END Magen plugin DOM -->
                        <div class="pt30 sub-email-div"><div class="pt30 sub-email-inside-div">

                          <div class="row" style="margin-top: 100px;">
                            <div class="col-md-3 col-md-offset-3 col-sm-offset-0">
                            </div>
                            <div class="col-md-6">
                              <form class="form-inline" style="margin: auto;">
                               <div class="input-group">
                                <input style="height:45px" type="email" class="form-control" size="100%" placeholder="Your Email Address" required>
                                <div class="input-group-btn">
                                  <button type="button" class="btn" style="background-color: black; color: white;">Subscribe</button>
                                </div>
                              </div>
                            </form>

                          </div>
                          <div class="col-md-3 col-md-offset-3 col-sm-offset-0">
                          </div>
                        </div><br>




                        <div class="row" style="margin-top:5px;">
                          <div class="col-md-3 col-md-offset-3 col-sm-offset-0" >
                          </div>

                          <div class="col-md-6" style="text-align: center;" >
                            <p><img src="assets/images/appStore.png" width="25%" height="50px" >
                              <img src="assets/images/windowStore.png" width="25%" height="50px" >
                              <img src="assets/images/googleplay.png" width="25%" height="50px" ></p></div>

                              <div class="col-md-3 col-md-offset-3 col-sm-offset-0">
                              </div> 



                            </div>


                            <div class="row" style="margin-top:5px;margin-bottom: -28px;">

                             <div class="col-md-4"><p style="padding-top: 5px;"><a href="mailto:contact@coinmarketcaptrader.com">contact@coinmarketcaptrader.com</a></p></div>

                             <div class="col-md-4 col-md-offset-4 col-sm-offset-0">
                              <p></p>
                            </div>

                            <div class="col-md-4"><p style="font-size: 9px; float: right; padding-top: 5px;">Copyright Protected<img src="assets/images/copy.jpg" width="20x"; height:"17px;"> 2020-3017 CMT CoinMarketcapTrader All Rights Reserved</p></div>


                          </div>


                        </div>
                        <!-- END Container -->

                      </section>
                      <!-- END Section -->


                    </div>
                    <!-- #### END Main content #### -->
                    <?php
                    require_once("footer.php");
                    ?>
                    <a id="back2Top" title="Back to top" href="#"><img src="assets/images/icontop.jpg" style="width: 33px; height: 33px;"></a>

                  </body>

                  </html>